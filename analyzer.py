...
SYSTEM_PROMPT = (
    ...
    "7. 박스권(레인지) 레짐일 때: 상단 저항에서 숏, 하단 지지에서 롱만 허용. 중간 구간 진입 금지. 레인지 돌파 거래량 확인 전 추세 추종 금지. 박스권 TP/SL은 피보나치가 아닌 직전 고점/저점 기준으로 설정하세요.\n"
    "\n"
...
• 진입 방식: [즉시 / 되돌림 / 돌파 후 진입]
  - 즉시: 현재가 진입
  - 되돌림: 매수면 현재가보다 낮게, 매도면 현재가보다 높게 (지정가 대기)
  - 돌파: 트리거 가격 +0.1% (매수) / -0.1% (매도)
• 진입가: $[숫자]
• 손절가: $[숫자]   ← SL 거리는 1h ATR 의 0.5~2.0 배 사이로 설정 (필수). 데이터의 [ATR] 섹션 참조.
• 목표가: $[숫자]   ← R:R 1.5 이상 필수. 즉 |목표가-진입가| ≥ 1.5 × |진입가-손절가|.
• 권장 레버리지: [숫자]배 (1~10)
