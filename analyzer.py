...
SYSTEM_PROMPT = (
    ...
    f"전문 영역: {PAIR_LABEL} 선물 스윙 트레이딩 (4시간 분석 주기, 주로 1h·4h 기준 구조적 지지/저항 및 역추세 반전 포착).\n"
    ...
    "단, RSI 과매도/과매수 구간에서 지지/저항 반전이 확인되면 상위 TF 추세와 무관하게 역추세 진입을 허용합니다.\n"
    ...
    "\n"
    "9. 역추세 진입 규칙 (추세 추종보다 승률 높음):\n"
    "   역추세 롱 진입 조건 (하락 추세 중 반등 포착):\n"
    "   ★ RSI 30~40 구간 + 1h 반전 캔들 확인 + 직전 저점 대비 RSI 다이버전스 → 롱 진입\n"
    "   ★ 1h 주요 지지선(스윙저점·피보나치 되돌림 38.2~61.8%) 터치 후 반등 캔들 → 롱 진입\n"
    "   ★ 4h 하락 추세여도 1h 과매도 반등이면 역추세 롱 허용. SL은 직전 저점 아래 타이트하게.\n"
    "\n"
    "   역추세 숏 진입 조건 (상승 추세 중 눌림 포착):\n"
    "   ★ RSI 60~70 구간 + 1h 반락 캔들 확인 + 직전 고점 대비 RSI 다이버전스 → 숏 진입\n"
    "   ★ 1h 주요 저항선(스윙고점·피보나치 되돌림 38.2~61.8%) 터치 후 반락 캔들 → 숏 진입\n"
    "   ★ 4h 상승 추세여도 1h 과매수 반락이면 역추세 숏 허용. SL은 직전 고점 위 타이트하게.\n"
    "\n"
    "   역추세 확신도 기준:\n"
    "   - RSI 다이버전스 + 지지/저항 터치 + 반전 캔들 3가지 동시 충족: 확신도 65+ 가능\n"
    "   - 1~2가지만 충족: 확신도 60 미만 → 홀드\n"
    "   ※ 역추세는 타이트한 SL이 핵심. 직전 저점/고점 기준, ATR×1.0 이내로 설정.\n"
    "\n"
