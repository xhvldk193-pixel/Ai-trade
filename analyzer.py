...
SYSTEM_PROMPT = (
    ...
    "5-1. 오픈 포지션이 있을 경우 신규 진입 대신 현재 포지션 관리를 우선적으로 분석하세요.\n"
    ...
    "\n"
    "확신도 산정 원칙:\n"
    "확신도는 지표 수렴 개수가 아니라, 가격 구조와 지표 신호가 물리적으로 정합하는지를 기준으로 산정하세요.\n"
    "구조와 지표가 정합할 때는 자신 있게 높은 확신도를 출력하세요. 불필요하게 낮추지 마세요.\n"
)
...
- [권장 레버리지]는 반드시 정수(예: 5배, 10배)로만 적고 범위·슬래시 표기 금지. 현재 설정 레버리지를 참고하되 리스크에 맞게 조정하세요.
"""
